...
assert(!bootstrap.includes('https://www.gstatic.com/firebasejs/'), 'Production bootstrap must not import the Firebase browser SDK.');
...
assert(indexHtml.includes('id="parserProvenancePanel"'), 'index.html must expose parser provenance before final clinical confirmation.');
assert(indexHtml.includes('id="printOperatorName"'), 'index.html must require a session-only print operator identity.');
